        ...
        if ns.run:
            ...
            while microscope.is_experiment_running and (not ns.stop):
                ...
                if ns.feedback_mode == "pid":
                    ...
                    dz = {
                        channel: np.clip(cyl.find_z(e, ns.q[get_cm_str(channel)]), -ns.max_step, ns.max_step)
                    ...
                    if np.any(np.isfinite(list(dz.values()))):
                        if ns.mult_channel_mode == 0:  # Average
                            ...
                            if np.isfinite(dz[ns.channels[cur_channel_idx]]):
                                zmem[ns.channels[cur_channel_idx]] = (
                                    piezo_pos + dz[ns.channels[cur_channel_idx]] * piezo_factor
                                )
                ...
                if xy_pos:  # Maybe move stage or ROI
                    ...
                    if ns.feedback_mode_xy == 1:
                        ...
                        microscope.move_stage_relative(
                            *np.clip(xy - ns.roi_size / 2, -ns.max_step_xy, ns.max_step_xy) * microscope.pxsize / 1e3
                        )

                # Wait for next frame:
                while (
                    microscope.time - 1 == time_n_now
                    and microscope.is_experiment_running
                    and not ns.stop
                    and not ns.quit
                ):
...
class UiMainWindow(QMainWindow):
    def __init__(self):
        ...
        self.warp_with_action.setStatusTip(
            "Save a copy of a file where the warp is corrected and use a specific bead or transform file to do so"
        )
...
class App(UiMainWindow):
    ...
    def calibrate(self, *args, **kwargs):
        if len(self.NS.channels) == 1:
            ...
            file, _ = QFileDialog.getOpenFileName(
                self, "Beads for calibration", "", "Carl Zeiss Image files (*.czi);;All files (*)", options=options
            )
            if file:
                self.calibz_thread = QThread(
                    cyl.calibrate_z,
                    self.calibrate_done,
                    file,
                    self.channels_box.textBoxValues,
                    self.NS.channels[:1],
                    self.NS.cyllens,
    ...
    def calibrate_done(self, result):
        try:
            ...
            self.conf[self.get_cyllens(result["channels"][0]) + result["magnification_str"]]["theta"] = float(
                result["theta"]
            )
            self.conf[self.get_cyllens(result["channels"][0]) + result["magnification_str"]]["q"] = result[
                "q"
            ].tolist()
    ...
    def warp_using_file(self):
        ...
        files, _ = QFileDialog.getOpenFileNames(
            self,
            "Bead files or transform file",
            "",
            "Carl Zeiss Image files (*.czi);;Transform files (*.yml);;All files (*)",
            options=options,
        )
    ...
    def warp(self, *, transform_files=None):
        options = QFileDialog.Options() | QFileDialog.DontUseNativeDialog
        files, _ = QFileDialog.getOpenFileNames(
            self, "Image files", "", "Carl Zeiss Image files (*.czi);;All files (*)", options=options
        )
    ...
    def run(self, *args, **kwargs):
        ...
        self.rectangle = self.microscope.draw_rectangle(
            *functions.clip_rectangle(
                self.microscope.frame_size, *self.NS.roi_pos, self.NS.roi_size, self.NS.roi_size
            ),
        ...
        while True:
            ...
            if not cfexists or (cfexists and not os.path.isfile(pfilename)):
                conf = {
                    ...
                    "FeedbackChannels": self.NS.channels,
                    "CylLens": self.NS.cyllens,
                    "mult_channel_mode": self.NS.mult_channel_mode,
                    "feedback_mode_xy": self.NS.feedback_mode_xy,
                    "DLFilterSet": self.duolink_block_drp.currentText(),
                    "DLFilterChannel": self.microscope.duolink_filter,
                    "maxStepxy": self.NS.max_step_xy,
                    "maxStep": self.NS.max_step,
                    "ROISize": self.NS.roi_size,
                    "ROIPos": self.NS.roi_pos,
                    "Columns": ["channel", "frame", "piezoPos", "focusPos", "x", "y", "fwhm", "i", "o", "e", "time"],
                }
                ...
                with open(pfilename, "w") as file:
                    ...
                    while (
                        (self.microscope.is_experiment_running or (not self.queue.empty()))
                        and (not self.stop)
                        and (not self.quit)
                    ):
                        ...
                        while (
                            self.microscope.is_experiment_running
                            and (not self.stop)
                            and (not self.quit)
                        ...
                        if not self.queue.empty():
                            ...
                            for channel in set(channels):
                                ...
                                if a.ndim > 1 and a.shape[1]:
                                    ...
                                    zfit = np.array(
                                        [
                                            -cyl.find_z(e, self.NS.q[self.get_cm_str(channel)]) if f else np.nan
                                    ...
                                    if channel == channels[0]:  # Draw these only once
                                        ...
                                        self.eplot.range_data(
                                            time_n, [self.NS.limits[channel][5][0]] * len(time_n), handle="bottom"
                                        )
                                        self.eplot.range_data(
                                            time_n, [self.NS.limits[channel][5][1]] * len(time_n), handle="top"
                                        )
                                        self.rplot.range_data(
                                            time_n, [self.NS.limits[channel][7][0]] * len(time_n), handle="bottom"
                                        )
