...
def localize(f, im, theta=None, fast_mode=False, progress=True):
    ...
    def fun(row):
        ...
        q, dq, r_squared = fn.fitgauss(
            im[int(h["C"]), int(h["Z"])], theta, sigma[int(h["C"])], fast_mode, True, h[["x_ini", "y_ini"]].to_numpy()
        )
...
def calibrate_z(file, em_lambdas, channels, cyllens=None, progress=None, elim=None, r2lim=None, path=None, theta=None):
    ...
    with Imread(file, axes="czyx") as im, PdfPages(f"{path}_cyllens_calib.pdf") as pdf:
        ...
        detections["r2"] = (detections["x"] - detections["x_ini"]) ** 2 + (detections["y"] - detections["y_ini"]) ** 2

        # individual Zhuang fits
        n_columns = 3
        n_rows = 5

        filtered = detections.query(
            f"R2>{r2lim[0]} & 0.1<s_um<0.6 & {elim[0]}<e<{elim[1]}"
            " & dx_um<0.2 & dy_um<0.2 & de<0.2 & ds_um<0.2 & r2 < 10"
        ).copy()

        fig.add_subplot(gs[:2, :5])
        imz = np.hstack([im[channel].max("z") for channel in channels])
        plt.imshow(imz, vmin=np.percentile(imz, 0.1), vmax=np.percentile(imz, 99.9))
        for i, channel in enumerate(channels):
            b = filtered.query(f"C=={channel}")[["particle", "x", "y"]].groupby("particle").agg("mean").reset_index()
        ...
        for particles in group(list(set(filtered["particle"])), n_rows * n_columns):
            ...
            for i, p in enumerate(particles):
                ...
                plt.errorbar(z[-1], sx[-1], yerr=dsx[-1], fmt=".m")
                ...
                if not np.isnan(px[-1]).all():
                    ...
                    plt.plot(Z, zhuang_fun(Z, px[-1]), "-m", Z, zhuang_fun(Z, py[-1]), "-g")
                    # plt.plot(Z, zhuang_fun(Z, px[i])/zhuang_fun(Z, py[i]), '-m')
                    plt.xlabel("z (um)")
                    plt.ylabel("s (um)")
                # plt.ylim(-0.5, 1.5)
                plt.ylim(0, 0.5)
        ...
        g = detections.query(
            f"R2>{r2lim[1]} & 0.1<s_um<0.6 & {elim[0]}<e<{elim[1]} & dx_um<0.2 & dy_um<0.2 & de<0.2 & ds_um<0.2"
        ).copy()
        ...
        (rl,) = plt.plot(Z, zhuang_ell(Z, q), "m-")
        E[E > elim[1]] = np.nan
        E[E < elim[0]] = np.nan
        Ze[Ze < -1] = np.nan
        Ze[Ze > 1] = np.nan
    ...
    return {
        "channels": channels,
        "magnification_str": magnification_str,
        "theta": theta,
        "q": q,
...
def error_plot(x, d):
    ...
    plt.plot(Z, error_fun(Z, p) - Z, "-m", Z, np.zeros(np.shape(Z)), "--k")
    ...
    plt.text(
        0.95,
        0.55,
        f"std(error): {1000 * np.std(dr):.0f} nm",
